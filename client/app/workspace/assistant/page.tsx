      ...
      if (response) {
        if (!activeConversationId && response.conversation_id) {
          ...
          setConversations((prev) => [
            { conversation_id: response.conversation_id, title: query.slice(0, 30) + "..." },
            ...prev,
          ]);
        }
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] w-full overflow-hidden bg-bg-primary text-text-primary">
      ...
      <div className="w-64 shrink-0 border-r border-border-subtle bg-bg-secondary flex flex-col h-full">
        ...
        <div className="p-3 border-b border-border-subtle flex items-center justify-between">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-accent text-white rounded-sm text-xs font-medium hover:bg-accent/90 transition-colors shadow-sm"
          >
            <Plus size={14} />
          ...
          {conversations.length === 0 ? (
            ...
            conversations.map((conv) => (
              <button
                ...
                className={cn(
                  "w-full text-left p-2.5 rounded-sm text-xs flex items-center gap-2 transition-colors",
        ...
        {/* Footer info */}
        <div className="p-3 border-t border-border-subtle text-[10px] font-mono text-text-tertiary flex items-center justify-between">
          <span>Port 8010 • LangGraph</span>
          <span className="text-accent font-medium">v1.0.0</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-bg-primary">
        {/* Top Control Toolbar */}
        <div className="h-12 border-b border-border-subtle px-4 flex items-center justify-between bg-bg-secondary shrink-0">
          <div className="flex items-center gap-2">
            <Bot size={18} className="text-accent" />
            <span className="text-sm font-semibold text-text-primary">Athleia Workforce Copilot</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-xs text-text-secondary">
              <span className="text-[11px] font-mono text-text-tertiary">Style:</span>
              <select
                value={explanationStyle}
                onChange={(e) => setExplanationStyle(e.target.value)}
                className="bg-bg-primary border border-border-subtle rounded px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-accent"
              >
                <option value="adaptive">Adaptive</option>
                <option value="beginner">Beginner (Simple)</option>
                <option value="technician">Technician (SOP/Physical)</option>
                <option value="engineer">Engineer (Deep Metrics)</option>
                <option value="manager">Manager (High-Level/Risk)</option>
              </select>
            </div>

            {/* Model Selector */}
            <div className="flex items-center gap-1.5 text-xs text-text-secondary">
              <span className="text-[11px] font-mono text-text-tertiary">Model:</span>
              <select
                value={selectedModel}
                onChange={(e) => setSelectedModel(e.target.value)}
                className="bg-bg-primary border border-border-subtle rounded px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-accent"
              >
                <option value="auto">Auto Router (Task-Based)</option>
                <option value="groq/llama-3.3-70b-versatile">Groq Llama 3.3 (Fast)</option>
                ...
                <option value="ollama/llama3.1">Local Ollama</option>
              </select>
            </div>
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1 px-2.5 py-1 rounded text-xs font-mono transition-colors border",
                mode === "deep_think"
            ...
            <div className="h-full flex flex-col items-center justify-center text-center max-w-xl mx-auto space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-base font-semibold text-text-primary">
                  How can Athleia Assistant support your work today?
                </h3>
                <p className="text-xs text-text-secondary leading-relaxed">
                  Ask operational queries, troubleshoot plant equipment, check ISO safety standards, or review predictive maintenance alerts. Grounded in enterprise knowledge.
                </p>
              </div>
                ...
                {QUICK_PROMPTS.map((qp, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className="p-3 rounded border border-border-subtle bg-bg-secondary hover:bg-bg-primary hover:border-accent/40 transition-all text-xs flex flex-col gap-1.5 group"
                    >
            ...
            messages.map((msg, idx) => (
              ...
              >
                ...
                <div className="flex-1 space-y-2 max-w-[85%]">
                  <div
                    className={cn(
                      "p-3.5 rounded-sm border text-xs whitespace-pre-wrap leading-relaxed shadow-sm",
                      msg.role === "user"
                  ...
                  {msg.tool_calls && msg.tool_calls.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {msg.tool_calls.map((tc, tIdx) => (
                        <div
                          key={tIdx}
                          className="px-2 py-0.5 rounded text-[10px] font-mono bg-bg-secondary border border-border-subtle flex items-center gap-1 text-text-secondary"
                        >
                  ...
                  {msg.citations && msg.citations.length > 0 && (
                    <div className="p-2.5 rounded border border-border-subtle bg-bg-secondary/50 space-y-1.5">
                      ...
                      <div className="space-y-1">
                        {msg.citations.map((c, cIdx) => (
                          <div key={cIdx} className="text-[11px] text-text-secondary bg-bg-primary p-2 rounded border border-border-subtle">
                  ...
                  {/* Feedback Controls (Assistant Messages) */}
        ...
        <div className="p-3 border-t border-border-subtle bg-bg-secondary shrink-0">
          ...
          >
            <input
              ...
              disabled={loading}
              className="flex-1 bg-bg-primary border border-border-subtle rounded px-3.5 py-2.5 text-xs text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading || !inputMessage.trim()}
              className="px-4 py-2.5 bg-accent text-white rounded text-xs font-medium hover:bg-accent/90 disabled:opacity-50 transition-colors flex items-center gap-1.5 shrink-0"
            >
              {loading ? (
                <>
